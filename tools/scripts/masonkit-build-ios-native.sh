#!/bin/sh
cd ../..
CWD="$(pwd)/packages/nativescript-masonkit/src-native"
NATIVE_SRC="$CWD/mason-native"
IOS_SRC_DIR="$CWD/mason-ios"
IOS_LIB_DIR="$IOS_SRC_DIR/Mason"
...
FEATURE_FLAGS="-Z build-std=std,panic_abort -Z build-std-features=panic_immediate_abort --package mason-ios"
...
if [[ -f "$IOS_LIB_INCLUDE/mason_native.h" ]]; then
  rm "$IOS_LIB_INCLUDE/mason_native.h"
fi
...
if [[ $IS_RELEASE == true ]]; then
  cd "$NATIVE_SRC"
 RUST_BACKTRACE=1 cargo +nightly build --target aarch64-apple-ios $BUILD_FLAG $FEATURE_FLAGS
  cp "$IOS_ARM_64_PHONE_OUTPUT_RELEASE_DIR" "$IOS_LIB_ARM_64_PHONE/$OUTPUT_LIB_NAME"
else
  cd "$NATIVE_SRC"
 RUST_BACKTRACE=1 cargo +nightly build --target aarch64-apple-ios $FEATURE_FLAGS
...
if [[ $IS_RELEASE == true ]]; then
  cd "$NATIVE_SRC"
  RUST_BACKTRACE=1 cargo +nightly build --target x86_64-apple-ios $BUILD_FLAG $FEATURE_FLAGS
 cp "$IOS_X86_64_SIM_OUTPUT_RELEASE_DIR" "$IOS_LIB_X86_64_SIM/$OUTPUT_LIB_NAME"
else
  cd "$NATIVE_SRC"
 RUST_BACKTRACE=1 cargo +nightly build --target x86_64-apple-ios $FEATURE_FLAGS
...
if [[ $IS_RELEASE == true ]]; then
  cd "$NATIVE_SRC"
 RUST_BACKTRACE=1 cargo +nightly build --target aarch64-apple-ios-sim $BUILD_FLAG $FEATURE_FLAGS
 cp "$IOS_ARM_64_SIM_OUTPUT_RELEASE_DIR" "$IOS_LIB_ARM_64_SIM/$OUTPUT_LIB_NAME"
else
  cd "$NATIVE_SRC"
 RUST_BACKTRACE=1 cargo +nightly build --target aarch64-apple-ios-sim $FEATURE_FLAGS
