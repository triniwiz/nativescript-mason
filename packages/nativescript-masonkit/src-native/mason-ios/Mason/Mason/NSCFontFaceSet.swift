...
//  Mason
//
//  Created by Osei Fortune on 04/05/2025.
//
...
public class NSCFontFaceSet: NSObject {
  ...
  internal let uiFontCache = NSCache<NSString, CTFont>()
  func font(for cgFont: CGFont, size: CGFloat) -> CTFont {
    let key = "\(cgFont.postScriptName ?? "default" as CFString)_\(size)" as NSString
