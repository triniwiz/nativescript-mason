...
/* Vendored copy of the mason-c C ABI for the Windows C++/WinRT component. Kept in sync with
   crates/mason-c (see crates/mason-windows). The component links masonnative.lib (built from
   crates/mason-windows) and calls only the cross-platform `mason_*` functions below — the
   apple-only helpers (`*_apple`, `mason_get_buffer`) are declared but never referenced on
   Windows, so they introduce no link dependency. */
