...
import org.nativescript.mason.masonkit.R
...
class DateInput(context: Context) : FrameLayout(context) {
  ...
  private fun clampAndNormalize(): String {
    ...
    val yrStr = String.Companion.format(Locale.getDefault(), "%04d", yInt)
    val monthStr = String.Companion.format(Locale.getDefault(), "%02d", mInt)
    val dayStr = String.Companion.format(Locale.getDefault(), "%02d", dInt)
  ...
  private val showButton = ImageButton(context).apply {
    background = null
      ...
      (this.context as? Activity)?.let {
        ...
        DatePickerDialog(
          ...
          { _, year, month, dayOfMonth ->
            // month is 0-based
            val yrStr = String.Companion.format(Locale.getDefault(), "%04d", year)
            val monthStr = String.Companion.format(Locale.getDefault(), "%02d", month + 1)
            val dayStr = String.Companion.format(Locale.getDefault(), "%02d", dayOfMonth)
  ...
  private fun createField(maxDigits: Int): EditText {
    return EditText(context).apply {
      ...
      filters = arrayOf(InputFilter.LengthFilter(maxDigits))
