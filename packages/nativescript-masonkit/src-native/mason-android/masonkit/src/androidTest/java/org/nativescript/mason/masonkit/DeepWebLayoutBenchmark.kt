...
class DeepWebLayoutBenchmark {
  ...
  fun profileDeepWebLayout() {
    ...
    val reportFile = File(instrumentation.targetContext.getExternalFilesDir(null) ?: instrumentation.targetContext.filesDir, REPORT_FILE)
  ...
  private class Fixture(context: Context) {
    ...
    init {
      root = flex(context, "root", FlexDirection.Column).apply {
        setPadding(24f, 20f, 24f, 28f)
        setGap(18f, 18f)
      }

      val header = flex(context, "header", FlexDirection.Row).apply {
      ...
      val page = flex(context, "page", FlexDirection.Row).apply {
      ...
      val sidebar = flex(context, "sidebar", FlexDirection.Column).apply {
        setSize(210f, 760f)
      ...
      val main = flex(context, "main", FlexDirection.Column).apply {
        flexGrow = 1f
        flexShrink = 1f
      ...
      repeat(6) { index ->
        val card = flex(context, "card-$index", FlexDirection.Column).apply {
      ...
      val article = flex(context, "article", FlexDirection.Column).apply {
        flexShrink = 0f
      ...
      val comments = flex(context, "comments", FlexDirection.Column).apply {
        setGap(14f, 14f)
      }
      ...
      repeat(2) { thread ->
        var parent = comments
        repeat(6) { depth ->
          val comment = flex(context, "thread-$thread/depth-$depth", FlexDirection.Row).apply {
            setGap(10f, 10f)
            setPadding(8f + depth, 7f, 8f, 7f)
            setMargin(if (depth == 0) 0f else 10f, 0f, 0f, 0f)
          }
          val rail = box(context, 3f, 54f)
          val content = flex(context, "thread-$thread/depth-$depth/content", FlexDirection.Column).apply {
            flexGrow = 1f
            flexShrink = 1f
            setMinSizeWidth(0f, Dimension.Kind.Points.value)
            setGap(5f, 5f)
          }
          val meta = flex(context, "thread-$thread/depth-$depth/meta", FlexDirection.Row).apply {
          ...
          val actions = flex(context, "thread-$thread/depth-$depth/actions", FlexDirection.Row).apply {
